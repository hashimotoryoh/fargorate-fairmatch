...
`NUXT_PUBLIC_RECAPTCHA_SITE_KEY` と `NUXT_RECAPTCHA_SECRET_KEY` は `/lookup` のBot対策に使う reCAPTCHA v3 のキー。[reCAPTCHA admin console](https://www.google.com/recaptcha/admin) でv3のサイトを登録して取得する。ドメインに `localhost` を加えれば、ローカル開発でも本番と同じ鍵ペアで動作を確認できる。

Googleが公開しているテスト用キー（`.env.example` に記載）は使わないこと。常にスコア0.9で成功するが `hostname` と `action` の検証をスキップする特殊なキーで、このアプリが行う `action` の一致チェックだけが常に失敗し422になる。
