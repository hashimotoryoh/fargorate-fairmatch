...
独自のIDもパスワードも持たないため、これは本人性を検証する「サインイン」ではない。FargoRate IDを知っていれば誰にでもなれる点は、この仕組みの前提として理解しておくこと。

FargoRate IDを持たないユーザーは `/guest` から、名前とレーティングを自己申告してサインインできる。名前の入力は任意で、省略すると「ゲストプレイヤー」として扱われる。自己申告のため信頼度は無く、リーグやチームの情報も表示されない。

未認証のユーザーが認証の必要なページを開くと `/link` へ送られ、サインインを終えると元のページへ戻る。パスワードの登録はない。
