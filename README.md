...
ユーザーは `/link` でFargoRateのプレイヤー情報とリンクする。入力された名前でFargoRateメンバーシップルックアップAPIを検索し、同姓同名を含む候補から、同じく入力されたFargoRate ID（メンバーシップID）の一致で1件に絞る。表示されたプレイヤーが自分であることをユーザーが確認すると、そのプレイヤーとしてセッションが始まる。FargoRateのAPIはメンバーシップIDでの検索を受け付けないため、名前での検索が唯一の経路である。

独自のIDもパスワードも持たず、名前とFargoRate IDを知っていれば誰にでもなれるため、本人性を検証する仕組みではない。この前提を踏まえ、UIとドキュメントでは「サインイン」という語を使わない。ただしセッションを終える操作は「サインアウト」のままである。

FargoRate IDを持たないユーザーは `/guest` から、名前とレーティングを自己申告して利用を開始できる。名前の入力は任意で、省略すると「ゲストプレイヤー」として扱われる。自己申告のため信頼度は無く、所在地の情報も表示されない。
...
外部APIへは実通信しない。FargoRateの応答はテスト側で差し替えている。
