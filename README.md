...
| `content/terms-conditions.md` | `/terms-conditions` |

ファイル名がそのままルートになるため、同じ仕組みで別のドキュメントも足せる。フロントマターには `title`・`description`・`updatedAt` を書く。`title` はページの見出しとタイトルタグに、`description` はメタタグに、`updatedAt` は最終更新日の表示に使う。本文の見出しは `##` から始め、`#` は使わない。
